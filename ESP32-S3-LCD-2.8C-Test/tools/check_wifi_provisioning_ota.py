...
def main() -> None:
    ...
    require(wifi_c, "http://47.108.204.22/api/ota/manifest?hardware=esp32s3", "primary OTA manifest URL")
    require(wifi_c, "http://60.205.122.153/api/ota/manifest?hardware=esp32s3", "fallback OTA manifest URL")
    require(wifi_c, "hardware=esp32s3", "status identifies S3 hardware for app package selection")
    require(wifi_c, "fetch_ota_manifest_url", "OTA manifest retries endpoint URLs")
